...
set_property IS_ENABLED 0 [get_drc_checks {AVAL-10}]
set_property IS_ENABLED 0 [get_drc_checks {REQP-34}]

foreach site [get_sites -of_objects [filter [roi_tiles] -filter {TYPE == "BRAM_L" || TYPE == "BRAM_R"}] -filter {SITE_TYPE == FIFO18E1}] {
	set cell [create_cell -reference FIFO18E1 ${site}_cell]
	lappend cells $cell
...
for {set i 10} {$i < 60} {incr i} {
	set FIFO_MODES [list "FIFO18" "FIFO18_36"]
	foreach cell $cells {
		set_property ALMOST_EMPTY_OFFSET "13'h[rand_offset]" $cell
		set_property ALMOST_FULL_OFFSET  "13'h[rand_offset]" $cell
		set_property SRVAL "36'h[randhex 9]" $cell
		set_property INIT  "36'h[randhex 9]" $cell
