...
foreach site [get_sites -of_objects [filter [roi_tiles] -filter {TYPE == "BRAM_L" || TYPE == "BRAM_R"}] -filter {SITE_TYPE == RAMBFIFO36E1}] {
	set cell [create_cell -reference RAMB36E1 ${site}_cell]
	lappend cells $cell
...
proc write_txtdata {filename} {
	...
	foreach cell $cells {
		...
		puts $fp "$tile $loc $init_a $init_b $srval_a $srval_b"
	}
	close $fp
}
...
for {set i 10} {$i < 40} {incr i} {
	foreach cell $cells {
		set_property SRVAL_A "36'h[randhex 9]" $cell
		set_property SRVAL_B "36'h[randhex 9]" $cell
		set_property INIT_A "36'h[randhex 9]" $cell
		set_property INIT_B "36'h[randhex 9]" $cell
	}
